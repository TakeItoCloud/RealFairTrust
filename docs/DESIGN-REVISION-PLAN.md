...
> **✅ COMPLETE (2026-06-24).** Run order **R1→R5 done** — plan → tokens → primitives → Home rebuild
> → global AA consolidation; all green + AA-recorded; **DECISIONS #57–#64 ratified**. The revision
> ships on the `chore/design-revision-*` chain and is **UNMERGED** — `main`+`develop` frozen at
> `04b6a1b` pending Carlos's promotion approval. See the §5 checklist (all ✅) + the 2026-06-24
> WORKLOG R2–R5 entries.
>
> **Status (history): PLAN ONLY at authoring — no app code changed in R1.** This is the gap
...
> 1→6, COMPLETE). The apply happened in green-gated steps **R2→R5** (§5/§6).
>
...
| # | Step | Scope | Status |
|---|---|---|---|
| **R2** | **Tokens + gold + type + champagne tokens/helpers + navy-side AA** | `app/design-tokens.css` + `app/globals.css`: flip `--bg-navy-radial` (C); both gold gradients + button-hover→§1.2 token (D); bump `--fs-hero` 76 / `--fs-section` 42 + add kit type aliases (`--fs-display-1/2`, `--fs-h1..h4`, `--font-*`, `--fw-*`) (B); add the champagne family + `.rft-champagne`/`.rft-step-card`/`.rft-step-coin` (E); add `champagne` tone to `SectionWrapper` + `Card`. **Then the navy-side AA re-measure+fix (§3.1)** — it changes immediately for *all* existing pages the moment the bg flips: muted-on-bright-centre, eyebrow gold, faint audit. Alias per §1; no component renames. | ✅ **Done 2026-06-24** (branch `chore/design-revision-tokens`). Resolved AA (measured vs worst-case `#1e4680`): **`--text-muted` .58→.70** (4.98 flat / 4.66 frosted card); **solid/eyebrow gold `--gold-500` #e3a812→#efb52a** (5.04 — one token fixes the eyebrow + ~10 small gold-text consumers; gradient `#e3a812` stops kept as literals); **`--champagne-eyebrow` #a9791a→#7c5a12** (4.90); verified-ink **#157048** carried fwd (4.74 champagne); ivory-label **#8C5E12** kept (5.32 ivory; N/A on champagne). Green: build+tsc+eslint ✅; smoke PT/EN 200 (EN→localized `/consultants`). **`SectionWrapper`/`Card` `champagne` tone DEFERRED to R4** (kept R2 token+helper-only per the deferral rule — helpers are inert until Home is wired). Full AA table in WORKLOG 2026-06-24 (R2). | 
| **R3** | **Primitives + cards value refresh** | Re-point Button (new 90° gold + sheen/focus per README §6.1), Card/StatBlock/Avatar/Badge/Tag/RankBadge/Input/Select to the new gradient/type values (mostly inherited); verify the gold **direction** reads correctly on `.gold-title`, `.btn-gold`, RankBadge coin, accent-bar; refresh `/dev/primitives` + `/dev/components`. AgentCard/PropertyCard pick up new gold/type automatically — verify against README §6.7/6.8 + `card-redesign.html`. | ✅ **Done 2026-06-24** (branch `chore/design-revision-primitives`). **Propagation verified** — zero hard-coded old gradients/gold/type in components; all gold reads canonical tokens (90° title/button, #efb52a small gold, 76/42 type, muted .70) automatically. **2 spec aligns:** Button → **pill** (`rounded-md`→`rounded-full`, §6.1/§2; a11y tap targets kept); Select `options` → `(string\|{value,label})[]` (§6.5; R4 hero pill). Card raised+custom padding+inline-style radius / StatBlock / RankBadge gold-glow / Avatar ring / Wordmark onIvory all verified (no change). Green build/tsc/eslint; smoke `/`·`/consultores`·`/[slug]`·`/en` 200 (pill+90° gold+42px live); dev showcases 200 under `pnpm dev` (prod-gated 404 by design). No new AA values (structural-only). | 
| **R4** | **Home composition rebuilt to the kit + champagne AA** | Rebuild `app/[locale]/page.tsx` + `components/home/*` to the kit Home (§4): search-pill hero + featured AgentCard + floating stat; champagne HowItWorks step-cards; navy Leaderboard + FeaturedProperties; navy AgentCTA (`--fs-display-2`); champagne Footer. Add/trim next-intl keys (PT+EN). **Champagne AA re-measure+fix (§3.2/3.3)** when the band first renders (champagne-eyebrow deviation; ivory-label relocation/moot). | ✅ **Done 2026-06-24** (branch `chore/design-revision-home`). Home rebuilt to the kit (hero search-pill + featured ConsultantCard + floating stat · champagne HowItWorks step-cards · navy Leaderboard/Featured · navy AgentCTA `--fs-display-2` · champagne Footer). Dropped split + trust-band sections (+ orphaned i18n keys); added hero/howItWorks/leaderboard/agentCta keys + footer.slogan/madeIn (pt+en parity). ConsultantCard `featured` prop; Eyebrow `tone="champagne"`. **AA at render all ≥4.5/≥3** — champagne-eyebrow #7c5a12 4.90, champ-ink-muted 5.89, step-card 9.44+, footer inks 11–13, CTA panel 7.4–12; **fixed** muted-on-raised-frosted @ bright centre 4.38→**4.66** (featured + floating stat → `.035` default fill, muted not lowered globally). Green build/tsc/eslint; smoke PT/EN 200, champagne only on HowItWorks+footer. Full table in WORKLOG 2026-06-24 (R4). | 
| **R5** | **Global AA consolidation + exception + supersession log** | Sweep all pages (Home, Consultores, Profile, dev) against the new bg/gold/type; finalize the AA exception table with **measured** ratios; log decisions #57+ (revision supersessions: bg, gold, type-B-over-#53(d), champagne-over-#56-ivory) in `docs/DECISIONS.md`; refresh `PROJECT-STATE.md` (§4/§5/§6/§7/§8) + `docs/WORKLOG.md`. | ✅ **Done 2026-06-24** (branch `chore/design-revision-consolidate`). Global AA sweep → found + fixed two brighter-bg regressions: on-dark green small TEXT (`--green-verified-strong #5fd2a1`, 4.58; energy cert + StatBlock delta) and tinted pill/badge chips → solid dark chip (green 6.78 / gold 9.05). Ratified **DECISIONS #57–#64**; refreshed PROJECT-STATE §4/§7/§8/§10/§12; green build/tsc/eslint + smoke 200. **Revision chain COMPLETE + UNMERGED.** Full AA table in WORKLOG 2026-06-24 (R5). |
