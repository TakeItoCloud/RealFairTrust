...
- **IN PROGRESS:** **Location hierarchy (CAOP2025) on discovery** built on `feat/location-hierarchy` (#80–#84; all gates green) — Distrito→Concelho→Freguesia picker + nearby fallback + area-specialist CTA; awaiting Carlos's Vercel-preview review before PR merge. **Two location models coexist** (CAOP + old Region) — unify later.
- **NEXT:** the remaining Phase 4.3 pages (Property detail `/imovel/[id]`, Vender, static pages — the Methodology page should also carry the DGT/CAOP attribution).
