...
**Merged to `develop`** (ahead of `main`; awaiting a future promotion to prod). `develop` HEAD = `a44b0e9` (Sobre + legal pages PR #14 — **Phase 4.3 public pages COMPLETE**):
...
- **NOTE:** `develop` is **ahead of `main`** by the 4.3 page PRs (#8/#9/#10 — Buy/Rent, location hierarchy, Property detail · #12 — Vender · #13 — Como funciona + Metodologia · #14 — Sobre + Termos + Privacidade); `main` still serves only the design-revision Home/Consultores/profile. **Phase 4.3 public pages are now COMPLETE on `develop`** → promoting `develop → main` is the **next decision, to be taken with Carlos** (Termos + Privacidade are DRAFTS — a Phase 6 legal-review blocker, not a promotion blocker).
