...
**Merged to `develop`** (ahead of `main`; awaiting a future promotion to prod). `develop` HEAD = `e2aafc2` (Vender PR #12):
...
**On `feat/static-como-metodologia`** (PR pending Carlos review — NOT merged): **Como funciona** `/como-funciona` (EN `/how-it-works`) — client-facing 3-step explainer (navy hero · champagne steps · weights strip 35/25/15/15/10 · green=verificação note · dual CTA) — and a **NEW Metodologia** page `/metodologia` (EN `/methodology`; new route #87) — long-form rating-model spec (§1–§6 navy/champagne bands · weights table · Rising-Talent/Building-track-record badge echo · **DGT/CAOP CC BY 4.0 attribution**). Decisions **#87/#88**.

**NEXT build, in order:**
1. **Remaining static pages** — **Sobre** (`/sobre`), **Termos** (`/termos`), **Privacidade** (`/privacidade`). *(Como funciona + Methodology now built — see above.)*
...
- **DONE:** **Como funciona** (`/como-funciona`) + **Metodologia** (`/metodologia`, NEW route #87) — **on `feat/static-como-metodologia`, PR pending Carlos review** (NOT merged). Client-facing 3-step explainer + long-form rating-model spec carrying the DGT/CAOP CC BY 4.0 attribution; confidence threshold stated qualitatively (#88); footer Metodologia link. #87/#88.
- **IN PROGRESS:** — (none; between tasks).
- **NEXT:** the remaining Phase 4.3 **static pages** — **Sobre** (`/sobre`), **Termos** (`/termos`), **Privacidade** (`/privacidade`).
- **NOTE:** `develop` is **ahead of `main`** by the four discovery PRs (#8/#9/#10/#12 — Buy/Rent, location hierarchy, Property detail, Vender); `main` still serves only the design-revision Home/Consultores/profile. Promoting `develop → main` is a separate step (with Carlos, likely once 4.3 is complete).
