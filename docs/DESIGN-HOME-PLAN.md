...
| **RH3b** | **HeroMedia + entrance + video** | Build `HeroMedia` (B.1, bleed only) + staged entrance w/ export-safety (B.2); optimize + wire `/videos/hero.mp4` + poster. | ⛔ **DEFERRED — blocked on tooling:** no `ffmpeg`/`ffprobe`/`avconv`/`imageio-ffmpeg` on the host, so the 8.37 MB / 5.0s mp4 can't be optimized (won't fake/ship it unoptimized). Resume once ffmpeg is available (or an optimized mp4 is provided). |
